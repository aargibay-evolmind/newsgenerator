...
<template>
  <div class="bg-[#F8FAFC] text-slate-900 font-sans flex flex-col min-h-screen pb-32">
    <MainHeader />
    ...
    <main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-8 py-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
        <div class="bg-white p-6 rounded-[2rem] border border-slate-200/60 shadow-sm flex flex-col gap-1">
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Artículos</span>
          <div class="flex items-end gap-2">
            <span class="text-3xl font-black text-slate-900 leading-none">{{ stats.count }}</span>
            <span class="text-xs font-bold text-slate-400 pb-0.5">generados</span>
          </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-slate-200/60 shadow-sm flex flex-col gap-1">
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tiempo de Lectura</span>
          <div class="flex items-end gap-2">
            <span class="text-3xl font-black text-slate-900 leading-none">~{{ stats.readingTime }}</span>
            <span class="text-xs font-bold text-slate-400 pb-0.5">minutos totales</span>
          </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-slate-200/60 shadow-sm flex flex-col gap-1">
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Actividad Hoy</span>
      ...
      <div class="flex flex-col md:flex-row gap-4 mb-8">
        <div class="flex-1 relative group">
          <div class="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            <svg class="h-4 w-4 text-slate-400 group-focus-within:text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input 
            ...
            class="w-full pl-11 pr-4 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all shadow-sm"
          />
        </div>
        <div class="flex gap-4">
          <select 
            v-model="sortBy"
            class="px-4 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-primary/20 shadow-sm cursor-pointer"
          >
      ...
      <div v-if="isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="i in 6" :key="i" class="bg-white rounded-[2rem] border border-slate-200/60 p-6 h-48 animate-pulse">
          <div class="h-4 w-3/4 bg-slate-100 rounded-full mb-4"></div>
          <div class="h-4 w-1/2 bg-slate-100 rounded-full mb-6"></div>
      ...
      <!-- Error State -->
      <div v-else-if="isError" class="p-8 bg-red-50 rounded-3xl border border-red-100 flex flex-col items-center text-center">
        <div class="h-14 w-14 bg-white rounded-2xl shadow-sm border border-red-100 flex items-center justify-center mb-4">
        ...
        <h3 class="text-lg font-bold text-red-900">Error de conexión</h3>
        <p class="text-red-600/70 text-sm mt-1 mb-6">No pudimos recuperar tus artículos en este momento.</p>
      ...
      <!-- Empty State -->
      <div v-else-if="!articles || articles.length === 0" class="text-center py-20 bg-white rounded-[3rem] border-2 border-slate-200 border-dashed shadow-sm">
        <div class="h-24 w-24 bg-slate-50 rounded-[2rem] flex items-center justify-center mx-auto mb-6">
      ...
      <div v-else-if="articles && articles.length > 0">
        ...
        <div v-if="searchQuery && filteredArticles.length > 0" class="mb-6 ml-2 animate-in fade-in slide-in-from-left-4 duration-500">
          <span class="text-xs font-bold text-slate-400">Mostrando {{ filteredArticles.length }} resultados para "{{ searchQuery }}"</span>
        </div>
        ...
        <div v-if="filteredArticles.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div 
            ...
            :key="article.id"
            class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 p-7 flex flex-col hover:shadow-xl hover:shadow-slate-200/50 transition-all cursor-pointer group relative border-b-4 hover:border-b-primary"
            @click="goToArticle(article.id)"
          >
            <!-- Metadata Badges -->
            <div class="flex items-center gap-2 mb-4">
              <span v-if="article.data?.tone !== undefined" class="px-2.5 py-1 bg-slate-100 text-[10px] font-black text-slate-600 rounded-full uppercase tracking-wider">
            ...
            <div class="flex justify-between items-start mb-6">
              <h2 class="text-xl font-black text-slate-900 group-hover:text-primary transition-colors line-clamp-2 leading-tight pr-6" :title="article.title">
                {{ article.title }}
              </h2>
              <button 
            ...
            <div v-if="article.data?.keywords && article.data.keywords.length > 0" class="flex flex-wrap gap-2 mb-8">
              <span 
                ...
                :key="kw"
                class="text-[10px] font-bold text-slate-400 border border-slate-100 px-2 py-0.5 rounded-lg"
              >
            ...
            <div class="mt-auto pt-6 flex items-center justify-between border-t border-slate-50">
              <div class="flex flex-col">
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Creado el</span>
                <span class="text-[11px] font-black text-slate-500">{{ new Date(article.created_at).toLocaleDateString() }}</span>
              </div>
              <div class="h-10 w-10 rounded-full bg-slate-50 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
        ...
        <div v-else-if="searchQuery" class="text-center py-24 bg-white rounded-[3rem] border border-slate-100 shadow-sm animate-in zoom-in-95 duration-300">
          ...
          <p class="text-slate-400 font-medium">No encontramos nada que coincida con "{{ searchQuery }}"</p>
